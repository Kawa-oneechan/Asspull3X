...
uniform vec4 sourceSize = vec4(640.0, 480.0, 640.0, 480.0);
uniform vec4 outputSize = vec4(640.0, 480.0, 640.0, 480.0);
uniform int phase;
...
{
	float2 size = sourceSize.xy;
	// align vertical coord to center of texel
	float2 uv = float2(p.x, (floor(p.y * sourceSize.y) + 0.5) / sourceSize.y);
#ifdef USE_DELAY_LINE
	float2 sh = (sourceSize.xy / sourceSize.xy / size) * float2(14. /10., -1.0);
#endif
	float2 pc = uv * sourceSize.xy / sourceSize.xy * size * float2(10., 1.);
	float alpha = dot(floor(float2(pc.x, pc.y)), float2(2., Phase_Y * 2.));
	alpha += Phase_One * 2.;

	// 1/size.x of screen in uv coords = sourceSize.x/sourceSize.x/size.x;
	// then 1/10*size.x of screen:
	float ustep = sourceSize.x / sourceSize.x / size.x / 10.;

	float border = sourceSize.x / sourceSize.x;
	float ss = 2.0;
#ifdef SWAP_VSIGN
...
#define PAL_SWITCH(A) A > 1.
#endif
	if (PAL_SWITCH(fmod(uv.y * sourceSize.y / sourceSize.y * size.y, 2.0)))
	{
		alpha = -alpha + 12012.0;
		ss = -2.0;
	}
	...
	float2 q = (p * sourceSize.xy / sourceSize.xy) * float2(TV_Pixels * 3., size.y * 2.);
#endif
	...
	float2 sw =
		saturate(min(float2(1., 2.), float2(bottom, bottom))
	...
	//float output_pixel_size = sourceSize.x / (outputSize.x * sourceSize.x);
...
void main() {
#ifdef USE_SAMPLED
	fragColor = vec4(monitor_sample(tex0, v_texCoord, outputSize.zw).rgb, 1.0);
#else
	fragColor = float4(monitor(tex0, v_texCoord), 1.);
#endif
}